...
TcpSocketBase::Bind (void)
{
  ...
  m_tcp->AddSocket (this); // Make a socket fully operational. Called after a socket has been bound, it is inserted in an internal vector.
...
TcpSocketBase::DoConnect (void)
{
  ...
  if (m_state == CLOSED || m_state == LISTEN || m_state == SYN_SENT || m_state == LAST_ACK || m_state == CLOSE_WAIT)
    { // send a SYN packet and change state into SYN_SENT
      ...
      if (m_tcb->m_useEcn == TcpSocketState::On) //ECN allows end-to-end notification of network congestion without dropping packets.
        {
...
TcpSocketBase::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  ProcessAck (tcpHeader, (bytesSacked > 0), currentDelivered, oldHeadSequence);
...
void
TcpSocketBase::ProcessAck(const TcpHeader &tcpHeader, bool scoreboardUpdated,
...
// Called by the ReceivedAck() when new ACK received and by SynRcvd()
...
TcpSocketBase::AddOptionSack (TcpHeader& header)
{
  ...
  uint8_t allowedSackBlocks = (optionLenAvail - 2) / 8; //how?
